...
class MGPR(gpflow.Module):
    ...
    @staticmethod
    def _cast_data(data):
        # gym returns float32 obs by default; GPflow expects default_float (float64).
        # Cast both X and Y so the GP's symbolic ops (e.g. d = x - mu in
        # log_marginal_likelihood) don't trip on a dtype mismatch.
        X, Y = data
        return np.asarray(X, dtype=float_type), \
               np.asarray(Y, dtype=float_type)
    ...
    def optimize(self, restarts=1):
        ...
        for model, optimizer in zip(self.models, self.optimizers):
            ...
            kv = model.kernel.variance.numpy()
            lv = model.likelihood.variance.numpy()
            if np.isfinite(loss) and np.all(np.isfinite(ls)) and \
                    np.all(np.isfinite(kv)) and np.all(np.isfinite(lv)):
                best_params = {"lengthscales": ls, "k_variance": kv, "l_variance": lv}
                best_loss = loss
            for restart in range(restarts):
                ...
                lv = model.likelihood.variance.numpy()
                if np.isfinite(loss) and np.all(np.isfinite(ls)) and \
                        np.all(np.isfinite(kv)) and np.all(np.isfinite(lv)) and \
                        loss < best_loss:
                    best_params = {"lengthscales": ls, "k_variance": kv, "l_variance": lv}
                    best_loss = loss
            if best_params is not None:
                try:
                    ...
                    import logging
                    logging.warning(
                        "mgpr.optimize: best params failed GPflow constraint check "
                        "(softplus inverse overflow); keeping current hyperparameters"
                    )
            else:
                import logging
                logging.warning("mgpr.optimize: all runs diverged (NaN/Inf); keeping current hyperparameters")
