...
class ExponentialReward(Module):
    ...
    def compute_reward(self, m, s):
        ...
        sR = r2 - muR @ muR
        muR = tf.ensure_shape(muR, [1, 1])
        sR = tf.ensure_shape(sR, [1, 1])
        return muR, sR
...
class CombinedRewards(Module):
    ...
    def compute_reward(self, m, s):
        ...
        # self.coefs is a gpflow.Parameter (symbolic tensor) — iterating over it
        # directly fails under tf.while_loop (graph mode) with
        # OperatorNotAllowedInGraphError. tf.unstack gives us a static-length
        # list of eager-iterable scalar tensors that the graph builder can handle.
        coefs = tf.unstack(self.coefs, num=len(self.base_rewards))
        for reward, coef in zip(self.base_rewards, coefs):
